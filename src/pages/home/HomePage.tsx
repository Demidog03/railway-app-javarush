...
import { Button, DatePicker, Flex, Form, InputNumber, Radio, Select, type FormProps, type InputNumberProps } from 'antd';
    ...
    useEffect(() => {
        if (stationsOptions.length > 0 && moscowStationsOptions.length > 0) {
            form.setFieldsValue({
                ...
                arrival: arrivalParam || moscowStationsOptions?.[0]?.value,
    ...
    return (
        <div className={classes.homePageContainer}>
            ...
            >
                ...
                <Flex gap="medium">
                    ...
                    <Form.Item name="arrival" rules={[{ required: true, message: 'Please select a arrival station' }]} style={{ flex: 1 }} label="Arrival">
                        <Select placeholder={isMoscowStationsLoading ? 'Loading...' : 'Select a arrival station'} options={moscowStationsOptions} />
                    </Form.Item>
                </Flex>
            ...
            <Flex style={{ marginTop: '50px' }} vertical gap="medium">
                <Button onClick={onClearFilters} size="middle" type="default" style={{ width: 'fit-content' }}>
                    Clear
                </Button>
